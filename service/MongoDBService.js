...
export class MongoDBService {
    ...
    async findByQuery(collectionName, pipeline, dbName) {
        try {
            const db = dbName ? this.client.db(dbName) : this.db;
            const collection = db.collection(collectionName);
    ...
    async paginationFormData(collectionName, pipeline, tenantid, page = 1, limit = 10, dbName) {
        try {
            const db = dbName ? this.client.db(dbName) : this.db;
            const collection = db.collection(collectionName);
    ...
    async searchFormData(collectionName, pipeline, tenantid, prefix = '', dbName) {
        try {
            const db = dbName ? this.client.db(dbName) : this.db;
            const collection = db.collection(collectionName);
    ...
    async aggregate(collectionName, pipeline, schema) {
        try {
